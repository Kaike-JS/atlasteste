...
function updateAppInterface() {
    ...
    animateCounters();
    updateChart();
    calculateInsights(displayTransactions);
...
window.exportToExcel = function () {
    ...
    const nomeMes          = selectMonth ? selectMonth.options[selectMonth.selectedIndex].text : 'Histórico';
    ...
    if (valorSelecionado !== 'all') {
        ...
        filtered = transactions.filter(t => {
            ...
            return d && new Date(d).getMonth() === mesIndex;
        });
    }

    if (filtered.length === 0) {
        showToast(`Nenhum lançamento em: ${nomeMes}.`, 'error');
        return;
    }
...
async function sendMonthlyReport() {
    ...
    if (transacoesFiltradas.length === 0) {
        showToast(`Nenhum lançamento em: ${nomeMesSelecionado}.`, 'error');
        return;
    }
